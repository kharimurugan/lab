...
SAVEDIR="/var/opt/BESClient/MaintenanceScripts/precheck/postchecks.${SAVEDATE}"
...
DATE=`date "+%d-%b-%Y-%H-%M"`
SERVER=`uname -n`
LOGDIR="/var/opt/BESClient/MaintenanceScripts/PREPOST_REPORT"
...
COMPAR_COUNT=`ls -ltr /var/opt/BESClient/MaintenanceScripts/PREPOST_REPORT | grep -v tmp_compare | wc -l`
while [ ${COMPAR_COUNT} -gt 4 ]
do
 OLD_DATA=`ls -ltr /var/opt/BESClient/MaintenanceScripts/PREPOST_REPORT | grep -v tmp_compare | grep -v total | head -1 | awk '{print $9}'`
 FOLDER="/var/opt/BESClient/MaintenanceScripts/PREPOST_REPORT/${OLD_DATA}"
 echo "$FOLDER"
 rm -rf $FOLDER
 COMPAR_COUNT=`ls -ltr /var/opt/BESClient/MaintenanceScripts/PREPOST_REPORT | grep -v tmp_compare | grep -v total | wc -l`
done

######################################################################
########################Validating the old data count#################

OLD_DIR=( $(ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck | grep -i prechecks. | awk '{print $9}' | tail -4) );
File_count=`ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[3]} | wc -l`
if [ $File_count -ge 10 ]; then
 echo "Number of files in ${OLD_DIR[3]} is $File_count"
 OLDDIR=/var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[3]}
else
 echo "No Valide data in latest Precheck direcory ${OLD_DIR[3]} and number of files in it is $File_count"
File_count=`ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[2]} | wc -l`
if [ $File_count -ge 10 ]; then
 echo "Number of files in ${OLD_DIR[2]} is $File_count"
 OLDDIR=/var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[2]}
else
 echo "No Valide data in latest Precheck direcory ${OLD_DIR[2]} and number of files in it is $File_count"
 File_count=`ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[1]} | wc -l`
if [ $File_count -ge 10 ]; then
 echo "Number of files in ${OLD_DIR[1]} is $File_count"
 OLDDIR=/var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[1]}
else
 echo "No Valide data in latest Precheck direcory ${OLD_DIR[1]} and number of files in it is $File_count"
File_count=`ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[0]} | wc -l`
if [ $File_count -ge 10 ]; then
 echo "Number of files in ${OLD_DIR[0]} is $File_count"
 OLDDIR=/var/opt/BESClient/MaintenanceScripts/precheck/${OLD_DIR[0]}
else
...
NEWDIR=/var/opt/BESClient/MaintenanceScripts/precheck/`ls -lrt /var/opt/BESClient/MaintenanceScripts/precheck | grep -i postchecks. | awk '{print $9}' | tail -1`
